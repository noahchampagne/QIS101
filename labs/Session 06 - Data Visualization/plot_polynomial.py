...
import numpy as np
from sympy import Derivative, Poly, lambdify, latex, real_roots, symbols
...
def plot(ax: plt.Axes):
    ...
    fn_sym = Poly([1, -2, -120, 22, 2119, 1980], x)

    # Find the real-only roots of the polynomial
    # and store in a numpy array of floats
    fn_zeros = np.asarray([float(r) for r in real_roots(fn_sym)])
    ...
    fn_d1 = Derivative(fn_sym, x, evaluate=True)
    ...
    fn_expr = fn_sym.as_expr()  # type: ignore
    fn_lambda = lambdify(x, fn_expr, modules="numpy")
